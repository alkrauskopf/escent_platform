class OrganizationType < ActiveRecord::Base
 ...
 def k12?
  ...
  def itl_summaries_between_dates_subject_belt(subj, month1, month2, belt)
       if subj
         ItlSummary.find(:all, :conditions => ["organization_type_id = ? AND subject_area_id = ? AND yr_mnth_of >= ? AND yr_mnth_of <= ?", self.id, subj.id, month1, month2]).select{|s| !s.itl_belt_rank.nil? && s.itl_belt_rank.rank_score >= belt.rank_score}
       else
         ItlSummary.find(:all, :conditions => ["organization_type_id = ? AND yr_mnth_of >= ? AND yr_mnth_of <= ?", self.id, month1, month2]).select{|s| !s.itl_belt_rank.nil? && s.itl_belt_rank.rank_score >= belt.rank_score}
       end
  end
  ...
  def tlt_sessions(subject, since_date)
    self.itl_organizations.collect{|o| o.tlt_sessions.for_subject(subject).since_date(since_date).size}.sum 
  end

  def tlt_session_time(subject, since_date)
    self.itl_organizations.collect{|o| o.tlt_sessions.for_subject(subject).since_date(since_date)}.flatten.collect{|s| s.session_length}.sum 
  end

  def tlt_session_count
    self.itl_organizations.collect{|o| o.tlt_sessions.size}.sum 
  end
