...
function iColorShow(id,id2){
	var eICP=jQuery("#"+id2).parents("li").position();
	jQuery("#iColorPicker")
		.css({'top':(eICP.top + 100)+(jQuery("#"+id).outerHeight())+"px",'left':(eICP.left + 520)+"px",'position':'absolute'})
		.fadeIn("fast");
...
for(i=0;i<hxs.length;i++){
	...
	var tblChilds=tbl.childNodes;for(j=0;j<tblChilds.length;j++){
