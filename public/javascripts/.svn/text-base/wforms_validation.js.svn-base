							...
							switch(arrClasses[j]) {
								...
								case "validate-custom": 
								  ...
								  // check for different delimiter
									var d_pattern = new RegExp("validate-custom\\s*m(.)");
									var d_matches = element.className.match(d_pattern);	
																		
									if (d_matches!=null) {	
									  var delimiter = d_matches[1];					
				  	        var pattern = new RegExp(delimiter+"([^]*)"+delimiter+"([gi]*)");
							      matches = element.className.match(pattern);
									  //alert("delimiter: "+matches[0]+"  |  "+matches[1]+"  |  "+matches[2]);  
									}
				          else {
