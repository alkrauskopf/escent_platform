...
// This software is licensed under the CC-GNU LGPL <http://creativecommons.org/licenses/LGPL/2.1/>
   ...
   if(wFORMS) {
		...
		wFORMS.arrMsg[0] 					= "Add another"; 					 // repeat link
			...
			evaluate: function(node) {
		   	...
		   	duplicateFieldGroup: function(e) {
				...
				if (element) {
					...
					var rowCount = parseInt(counterField.value);
					// Prepare id suffix (RAILS Version)
					...
					// Save new row count 			
					document.getElementById(element.id + wFORMS.idSuffix_repeatCounter).value = rowCount + 1;
					...
					if(!document.all || window.opera) { 
						...
						if(element.name) 					
							if (element.tagName.toUpperCase() == "INPUT" && 
								...
								tagHtml += " NAME='" + wFORMS.behaviors['repeat'].removeRepeatCountSuffix(element.name);
						if(element.type) {
